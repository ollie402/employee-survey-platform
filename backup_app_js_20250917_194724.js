        ...
        for (const recipient of recipients) {
            try {
                ...
                if (emailResponse.ok) {
                    ...
                    console.log(`Update details: invite_sent=true, reminder_count=${reminderCount}, date=${currentDate}`);

                    // First attempt to update the database
                    ...
                    try {
                        ...
                        if (error) {
                            ...
                            // Try updating by email and survey_id as fallback
                            console.log('🔄 Trying fallback update method using email and survey_id...');
                        ...
                        console.error('❌ Database update threw an exception:', dbError);
                        updateError = dbError;
                    }

                    if (updateSuccess) {
                        console.log('📊 Update result:', updateData);
                        console.log('📋 Updated record:', updateData && updateData.length > 0 ? updateData[0] : 'No data returned');
                        ...
                        console.error('❌ Database update failed for:', recipient.email);
                        console.error('Error details:', updateError);
                        showToast(`Database update failed for ${recipient.email}. Email sent but status not updated.`, 'warning');
                    }
                } else {
                    console.error(`❌ Failed to send email to ${recipient.email}`);
                    const emailResult = await emailResponse.text();
                    console.error('Email API response:', emailResult);
                    showToast(`Failed to send email to ${recipient.email}`, 'error');
                }
            } catch (error) {
